...
import { useGetAllQrCodes, useGetCourses } from "@/app/(protected)/(lecturer)/qr-generation/queries";
...
export function AttendanceTrackingScreen() {
  ...
  const sessionsWithCourses = useMemo((): SessionWithCourse[] => {
    ...
    return allQrCodes.data.map(sessionData => {
  ...
  const currentLiveSession = useMemo((): (SessionWithCourse & {
                ...
                <Card>
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-muted-foreground">
                      No QR code sessions found. Generate your first QR code to get started.
                    </p>
                  </CardContent>
                </Card>
              ) : (
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {sessionsWithCourses
                    .sort((a, b) => {
                      ...
                      const aIsLive = currentLiveSession?.study_session_id === a.study_session_id;
                      const bIsLive = currentLiveSession?.study_session_id === b.study_session_id;
                      ...
                      if (aLatestQr && bLatestQr) {
                        return new Date(bLatestQr.createdAt).getTime() - new Date(aLatestQr.createdAt).getTime();
                      }
                    ...
                    <SessionCard
                      ...
                      session={session}
                      isLive={currentLiveSession?.study_session_id === session.study_session_id}
                      onClick={() => {
                        const latestQrCode = session.qr_codes[session.qr_codes.length - 1];
                        handleSessionClick(session.study_session_id, latestQrCode?.week_number);
                      }}
                    />
                  ))}
                </div>
              ...
              {currentLiveSession ? (
                <Card className="border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/20">
                  ...
                  <CardContent>
                    <div className="grid gap-2">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span>{currentLiveSession.start_time} - {currentLiveSession.end_time}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm">
                        <QrCodeIcon className="h-4 w-4 text-green-600" />
                        <span className="text-green-600">
                          QR Code Active (Week {currentLiveSession.activeQrCode.week_number})
                        </span>
                      </div>
                      <div className="text-xs text-green-600">
                        Valid until {new Date(currentLiveSession.activeValidity.end_time).toLocaleString()}
                      </div>
                    </div>
                  </CardContent>
                  <CardContent className="pt-0">
                    <Button
                      ...
                      onClick={() => handleSessionClick(currentLiveSession.study_session_id, currentLiveSession.activeQrCode.week_number)}
                    >
                ...
                <Card>
                  <CardContent className="p-6 text-center">
                    <QrCodeIcon className="mx-auto h-12 w-12 text-muted-foreground mb-4" />
                    <p className="text-muted-foreground">
                      No live sessions currently active. QR codes will appear here when they are within their validity window.
                    </p>
                  </CardContent>
                </Card>
              )}
    ...
    >
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            {session.qr_codes.length} QR code{session.qr_codes.length !== 1 ? 's' : ''} generated
          </span>
        </div>
        ...
        {latestQrCode && (
          <div className="text-xs text-muted-foreground">
            ...
            {latestQrCode.validities.length > 0 && (
              <span className="ml-1">
                ({latestQrCode.validities.length} validit{latestQrCode.validities.length !== 1 ? 'ies' : 'y'})
              </span>
            )}
          </div>
        )}
        ...
        {latestQrCode && (
          <div className="text-xs text-muted-foreground">
            Created: {new Date(latestQrCode.createdAt).toLocaleDateString('en-US', {
              year: 'numeric',
              month: 'short',
              day: 'numeric',
              hour: '2-digit',
