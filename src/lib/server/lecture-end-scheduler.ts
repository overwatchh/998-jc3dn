...
async function triggerLectureEndEmail(
  sessionId: number,
  weekNumber: number = 1
) {
  console.log(
    `🎯 Automatic lecture end trigger for session ${sessionId}, week ${weekNumber}`
  );

  try {
    const response = await fetch(
      `http://localhost:3000/api/system/lecture-end-trigger`,
      {
        method: "POST",
        headers: {
...
export function scheduleSessionEnd(
  ...
  console.log(
    `⏰ Scheduling session ${sessionId} to end at ${endTime} on ${dayOfWeek} with pattern: ${cronPattern}`
  );
  ...
  const task: ScheduledTask = cron.schedule(
    cronPattern,
    () => {
      void triggerLectureEndEmail(sessionId, weekNumber);
    },
    {
...
export async function initializeLectureEndScheduler() {
  ...
  try {
    ...
    await fetch(`http://localhost:3000/api/system/init`);
    ...
    cron.schedule("* * * * *", () => {
      void checkForEndedSessions();
    });

    console.log(
      "✅ Automatic lecture end scheduler initialized (dynamic mode)"
    );
  } catch (error) {
...
async function checkForEndedSessions() {
  ...
  if (commonEndTimes.includes(currentTime)) {
    console.log(
      `🎯 Checking for sessions ending at ${currentTime} on ${currentDay}`
    );

    // Try to trigger for any active sessions (this is a temporary approach)
    // Fire-and-forget sample session triggers (ignore errors intentionally)
    for (const sessionId of [101, 102, 103]) {
      void triggerLectureEndEmail(sessionId, 1);
    }
  }
}
